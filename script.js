...
async function renderUserCategories() {
  ...
  // جلب كافة المطاعم لحساب الأعداد
    ...
    container.innerHTML += `
      <div class="category-card" data-ar="${cat.titleAr}" data-en="${cat.titleEn}" onclick="openRestaurants('${cat.titleAr}', '${cat.titleEn}', '${cat.imgUrl}')">
        ...
        <div class="cat-info">
          ...
          <div class="cat-title-en">${cat.titleEn} • <span style="color: var(--primary-teal, #0f4c5c); font-weight: bold;">${count} مطعم</span></div>
        </div>
...
async function saveCategoryToFirebase() {
  ...
  let catData = {
    ...
    order: docId ? undefined : (currentCategoriesList.length + 1),
    updatedAt: serverTimestamp()
  };

  if (docId) delete catData.order;
