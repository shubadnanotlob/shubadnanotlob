...
async function saveRestaurantToFirebase() {
  ...
  let restData = {
    ...
    logo: logo || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=500&q=80',
...
function renderRestaurants(list) {
  ...
  list.forEach(rest => {
    // التأكد من استخدام rest.logo مع حماية للروابط المكسورة
    const logoSrc = (rest.logo && rest.logo.trim() !== '') ? rest.logo : currentHeroImage;
    
    container.innerHTML += `
    <div class="restaurant-card">
      <div class="rest-top-content">
        <img class="rest-icon" src="${logoSrc}" alt="Logo" onerror="this.onerror=null; this.src='${currentHeroImage}';">
        <div class="rest-info">
          <h4>${rest.name}</h4>
          <p>${rest.desc}</p>
        </div>
      ...
      <div class="rest-actions">
        <a href="${rest.menuUrl}" target="_blank" class="action-btn btn-menu">MENU</a>
        <a href="https://wa.me/${rest.phone}" target="_blank" class="action-btn btn-contact">CONTACT</a>
        <a href="${rest.mapUrl}" target="_blank" class="action-btn btn-location">LOCATION</a>
      </div>
    </div>
    `;
  });
